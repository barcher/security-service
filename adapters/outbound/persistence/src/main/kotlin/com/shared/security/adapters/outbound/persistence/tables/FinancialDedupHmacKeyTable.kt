...
/** Maps to `V8__financial_dedup_hmac_key.sql`. */
object FinancialDedupHmacKeyTable : Table("financial_dedup_hmac_key") {
    val id = char("id", length = ID_LENGTH)
    val version = integer("version")
    val status = enumerationByName<FinancialDedupHmacKeyStatusValue>("status", length = STATUS_COLUMN_LENGTH)
...
enum class FinancialDedupHmacKeyStatusValue {
    ACTIVE,
    RETIRED,
}
