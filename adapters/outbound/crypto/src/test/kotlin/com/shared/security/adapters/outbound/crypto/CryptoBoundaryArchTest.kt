...
class CryptoBoundaryArchTest {
    private val cryptoClasses =
        ClassFileImporter()
            ...
            .importPackages("com.shared.security.adapters.outbound.crypto")

    @Test
    fun `S-1 crypto module must not depend on domain`() {
        noClasses()
            .that().resideInAPackage("com.shared.security.adapters.outbound.crypto..")
            .should().dependOnClassesThat()
            .resideInAPackage("com.shared.security.domain..")
    ...
    fun `S-2 crypto module must not depend on persistence adapter`() {
        noClasses()
            .that().resideInAPackage("com.shared.security.adapters.outbound.crypto..")
            .should().dependOnClassesThat()
            .resideInAPackage("com.shared.security.adapters.outbound.persistence..")
            .allowEmptyShould(true)
            .check(cryptoClasses)
    }
