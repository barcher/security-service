...
> Re-graded at each stream's close. **Last update: 2026-05-28 — financial-dedup blind-index endpoint (`POST /v1/blind-index/financial-dedup` + `financial_dedup_hmac_key` V8); legacy KEK retirement (cooldown); monolith data DEK re-wrapped off `LEGACY-V0` onto the current KEK.**
>
...
> - **Stream L L.1+L.2+L.3 complete on the monolith side (2026-05-24):** consumer adoption of the L.0 observability surface. L.1 — new `adapters/outbound/security-readonly/` submodule (`SecurityServiceReadOnlyPort` + `Remote/LocalDev/NoOp` adapters via mode-aware DI resolver; M-14/M-15/M-16/M-17 + M-12b ArchUnit ratchets; dedicated `SECURITY_SERVICE_READONLY_URL` + observer-cert env-var lane independent of operational + admin lanes). L.2 — `GetCrossServiceKeyStatusUseCase` composes cross-service KEK/DEK observation with monolith-local `column_encryption_config`; `SearchCryptoAuditLogsUseCase` proxies the security-service audit-log query; `GET /api/security/key-status` and new `GET /api/security/crypto-audit-logs` route handlers; `PUT /api/security/settings` rejects (422) writes to `keyRotationIntervalHours` and `encryptionHealthCheckIntervalMinutes` per Shared-Key-Service rule 22. L.3 — frontend re-skin: `/security/keys` page swaps banner copy to point at `SECURITY_SERVICE_READONLY_URL` and renders the new `CrossServiceErrorBanner`; `/security/audit-logs` adds a "Business events" / "Crypto-ops" tab toggle reading the two physically distinct HMAC chains; `/security/settings` converts the two security-service-owned fields to read-only badges and tightens `updateSecuritySettings` to an options-bag accepting only `auditLogQueryTimeoutSeconds`. Posture grades unchanged — this is consumer adoption of the existing Observability surface, not a new wire/auth/audit surface.
> - **Stream L L.4 complete on the monolith side (2026-05-24 — operator gate overridden per user direction):** `KeyRotationPort` + `KekRotationPort` + the dead `KeyRotationJob` deleted; the monolith holds NO in-process rotation handles. `DekWrappedStringEncryptor`'s implements list collapsed to `StringEncryptorPort + ColumnEncryptionPort`; rotation method bodies + the `reEncrypt*` helper family + `StringTableEncConfig` + the now-unused `adminPort` constructor param deleted. `/api/security/encryption/rotate-kek` HTTP route deleted along with its DTOs — operator KEK rotation is a security-service HSM ceremony (`HSM_KEY_CEREMONY.md`), not a one-click monolith web action. Monolith `key-status` route falls through to a degraded-mode empty page populated from `ColumnEncryptionPort.getColumnEncryptionConfig()` when the cross-service use case is unwired. New monolith ArchUnit ratchet **M-18** (`CryptoModuleOutwardBoundaryTest.kt`) guards re-introduction of either deleted port with class-level FQN checks + a source-grep companion (`STREAM_L_LEGACY_EXEMPTIONS = emptySet()`). `meta-project/CLAUDE.md` "Shared Key Service" current-state flipped to "post-Stream-L". Posture grades unchanged on the security-service side — L.4 is a monolith-side deletion, not a security-service change; the four-lane subject-DN model (operational / admin / dashboard-observer / future operator-decrypt) is unaffected.
> - **Stream M M.0+M.1+M.2 complete (2026-05-24 — single-sided per `feedback_operator_decrypt_cli_single_sided.md`):** standalone operator decrypt CLI lands in `security-service/tools/decrypt-cli/`. New audit event type `OPERATOR_DECRYPT_RUN` emitted BEFORE any unwrap (CLAUDE.md rule 26). New `AuditLogQueryPort.findById(id)` method + Exposed implementation for the audit-event-id scope. 14-flag argument surface (locked at M.0 sign-off in `ParityArg.kt` + `operator_decrypt_output.schema.json` `schema_version="1.0"`). Cert subject DN locked at `CN=workautomations-operator-decrypt-<16-hex>,O=WorkAutomations` with SHA-256 email hash (CLAUDE.md rule 28 — never changed; 7-year FedRAMP AU-11 retention depends on the format). New env var `SECURITY_OPERATOR_DECRYPT_SUBJECTS` separates the operator allow-list from `SECURITY_ADMIN_SUBJECTS` + `SECURITY_DASHBOARD_OBSERVER_SUBJECTS`. 33 unit tests cover argument parsing, DN validation, hard-cap enforcement, and orchestration (audit-row-before-unwrap, hard-cap-rejects-without-audit, dry-run-no-rows, audit-event-id scope). ArchUnit ratchets `StreamMBoundaryArchTest`: **S-17** (no inbound HTTP DTO has `val plaintext:` / `val decryptedValue:` field), **S-18** (no infrastructure class depends on the CLI module), **S-19** (no SLF4J call inside the CLI references plaintext/decrypted/clear/unwrapped identifiers — strict from day one). Monolith-side forward guards `CryptoModuleOutwardBoundaryTest`: **M-19** (no `application/` use case combines `unwrapDek`/`decrypt` with time-window OR ID-list signature) + **M-20** (no monolith HTTP route adds `/admin/decrypt-*` / `/security/decrypt-*` literal). M.2 wires `ProductionCliRuntime.boot()` using the same `SECURITY_DB_*` + `AUDIT_HMAC_KEY` env-var conventions as the running service; CLI runs from `installDist` output on operator workstation only (never bundled into `security-app` docker image per CLAUDE.md rule 24). Operator runbook (`OPERATOR_DECRYPT_RUNBOOK.md`) shipped — cert minting, allow-list setup, usage, audit semantics, exit codes, rehearsal flow, troubleshooting. AUDIT_LOG.md catalog + docs allowlist updated. Posture grades unchanged — the CLI does not introduce a new wire/auth surface (it reuses `/v1/dek/unwrap` from inside the operator workstation); it eliminates a previously-diffuse "engineer SSHes in and decrypts a row" pattern by replacing it with a single audited entry point.
> - **Email blind-index endpoint (2026-05-27):** new operational-lane route `POST /v1/blind-index/email` lets the monolith compute a one-way 16-byte HMAC-SHA-256 blind index over a normalized email (`trim().lowercase()`) without the HMAC key ever leaving the security service. Backs the monolith's `company_candidates.email` encryption-at-rest migration: equality lookups (`findByEmail`) match on the blind index instead of plaintext. New `email_lookup_hmac_key` table (V7) stores the 32-byte key KEK-wrapped (AAD `email-lookup-hmac-key:<version>`), generated on first use, singleton-ACTIVE invariant via generated-column + unique index (same idiom as `keks`/`jwt_signing_keys`). Key is unwrapped once + cached in-process (unwrapping per-request would put the KEK on the monolith's findByEmail hot path). `ComputeEmailBlindIndexUseCase` + `ExposedEmailLookupHmacKeyRepository` + DI wiring. **No audit row per call** (routine, high-volume, one-way) and **no plaintext logging** — enforced by ArchUnit **S-22** (no logger reference in `BlindIndexRoutes.kt`). Endpoint sits behind the existing mTLS operational lane; it is NOT public (not added to `publicPathPrefixes`, so S-20 is unaffected). Posture grades unchanged — same TLS/auth surface as `/v1/dek/*`; no new key-material exfiltration path (the key stays server-side, only the truncated MAC crosses the wire).
> - **Financial-dedup blind-index endpoint (2026-05-28):** second operational-lane blind-index route `POST /v1/blind-index/financial-dedup` backs financial-service's transaction dedup index. The route returns `HMAC-SHA-256(key, prehash)` (full 32 bytes) over a caller-supplied **32-byte SHA-256 PREHASH** — NOT a raw value. This is the deliberate difference from the email route: the financial dedup preimage folds in the transaction's merchant token, which is encrypted at rest in `financial_transactions.merchant_name_encrypted`; sending it raw would defeat that boundary. financial-service prehashes the natural-key preimage locally so the merchant token never crosses the wire. New `financial_dedup_hmac_key` table (V8) stores the 32-byte key KEK-wrapped (AAD `financial-dedup-hmac-key:<version>`, distinct namespace from `email-lookup-hmac-key:<version>` — one purpose's key can never compute another's indices), generated on first use, singleton-ACTIVE invariant via generated-column + unique index (same idiom as `keks`/`jwt_signing_keys`/`email_lookup_hmac_key`). Key unwrapped once + cached in-process (per-request unwrap would put the KEK on the transaction-import hot path). `ComputeFinancialDedupBlindIndexUseCase` + `ExposedFinancialDedupHmacKeyRepository` + DI wiring; the prehash is validated to be exactly 32 bytes (422 `prehash_must_be_sha256` otherwise). **No audit row per call** (routine, high-volume, one-way) and **no plaintext logging** — same ArchUnit **S-22** guard (no logger reference in `BlindIndexRoutes.kt`). The caller is the financial-monolith operational-lane subject (`CN=workautomations-financial-monolith`); the route is NOT public. **Rotation note:** the key is expected NEVER to rotate in normal operation — rotation invalidates every stored dedup blind index and forces an app-layer recompute of every `financial_transactions` row (the merchant is encrypted, so SQL cannot recompute the index); `version` + the RETIRED status exist only to support a future dual-hash rotation window. Posture grades unchanged — same TLS/auth surface as `/v1/blind-index/email`; the key stays server-side, only the MAC crosses the wire.
> - **Stream C follow-up SHIP-01+02+05+06 partial (2026-05-24):** the `AuditLogShipperJob` + `AuditRetentionJob` + `KekBackupVerifyJob` + `KekRotationHealthJob` + `KekPriorTtlJob` + `DekRotationJob` Quartz wiring lands. `NoOpColdStorageAdapter` + `NoOpKekBackupVerifier` bound to their ports in DI; the 6 use cases + their dependency closures (chain-verifier + batch-reader + checkpoint provider/saver + deleter) wired against `ExposedAuditLogRepository` + new `AuditShippedCheckpointRepository`. New Flyway migration `V6__audit_shipped_checkpoint.sql` adds the single-row checkpoint table the shipper + retention jobs share. `SecurityScheduler` registered in Koin and `.start()`/`.stop()` lifecycle hooked into `Application.securityModule()` via `ApplicationStopping`. `SECURITY_SCHEDULER_ENABLED` defaults to `false` so prod doesn't unexpectedly start firing jobs against the NoOp cold-storage adapter (would silently drop audit batches while the retention job advances — hard data-loss risk). Operator runbook `AUDIT_SHIPPER_RUNBOOK.md` documents wiring state, decision matrix, env vars, cadence tuning, monitoring, DR flow, and the SHIP-03/04 follow-up work still needed before enabling. **SHIP-03 + SHIP-04 (real R2/S3 cold-storage + backup-verifier adapters) DEFERRED** — operator decides vendor first. `Backup verification` posture row stays at **C** until SHIP-04 ships a real verifier; bumping to A is the SHIP-04 exit criterion.
> - **Legacy KEK retirement — cooldown (2026-05-28):** the monolith data DEK (`encryption_keys`) was re-wrapped off the unwrap-only legacy KEK (`ML-KEM-768/AES-256-GCM-LEGACY-V0`) onto the current KEK via `meta-project/scripts/rewrap-monolith-deks-to-current-kek.sh` (operational-lane unwrap+wrap, in place, DEK bytes unchanged). A cross-service audit confirmed ZERO `LEGACY-V0`-wrapped material remains (monolith `encryption_keys`, financial `financial_vault_keys`, security `deks`/`keks`/`jwt_signing_keys`). `ML_KEM_*_LEGACY_V0` is commented out in the security-service `.env` for a cooldown window — startup is unaffected (current KEK still ACTIVE; `MlKemService.fromLegacyEnv()` returns null → `hasLegacyKek=false`), and `/v1/dek/unwrap` only errors for a `LEGACY-V0`-tagged request, of which none remain. The legacy-unwrap code path is intentionally retained for the cooldown; full deletion of the credential values + the `fromLegacyEnv` path is a tracked fast-follow after the window closes. Posture grades unchanged (fewer live key lineages — strictly posture-positive).
...
| **Authentication** | A | mTLS on every endpoint **except the two documented public routes** (`GET /v1/jwks`, `GET /v1/health`). **Two-connector topology** (post-2026-05-26): `Application.kt::buildServer` binds an mTLS-required `sslConnector` on `SECURITY_SERVICE_PORT` (default 8443) with both keystore + truststore — Ktor sets `needClientAuth=true` so anonymous handshakes are rejected at the TLS layer. A second `sslConnector` on `SECURITY_SERVICE_PUBLIC_PORT` (default 8442) uses the same keystore with NO truststore — anonymous handshakes succeed there, but the `MtlsAuthPlugin` allow-list (`publicPathPrefixes = setOf("/v1/jwks", "/v1/health")`) ensures only those two routes are reachable; every other route returns HTTP 401 + `MTLS_REJECTED` audit. Earlier `wantClientAuth` mutation attempts were abandoned: Ktor 3.1.1's `NettyApplicationEngine` defers `channelPipelineConfig` until after ALPN, which happens during the handshake — too late to mutate the engine. Splitting connectors makes each `SslHandler`'s mode correct at construction. **Stream D**: monolith-side client is wired via `RemoteCryptoKeyServiceAdapter` — TLS 1.3 only, ECDSA P-384, exponential-backoff retry, fail-closed on connection error. `DenyAllPeerCertChainExtractor` remains as the fail-closed fallback when `MtlsConfig.fromEnv()` returns null. |
| **Authorization** | A | Admin endpoints gated on `AdminAllowList` subject DN; ADMIN_FORBIDDEN audit on every reject. |
| **Rate limiting** | B+ | Per-subject token bucket on `/v1/dek/unwrap` and `/v1/observability/*`. Per-remote-IP bucket on the public `/v1/jwks` route (default 60 burst, 10/s refill; over-cap → 429). **Caveat:** local-process state — multi-replica scale-out requires a shared limiter (Stream F follow-on). |
...
|---------|----------------|-----------|
| mTLS gate before routing | `installMtlsAuth` interceptor in `Plugins` phase, with an explicit `publicPathPrefixes` allow-list (currently `/v1/jwks`, `/v1/health`). Pairs with a two-connector topology in `buildServer`: mTLS-REQUIRE on `SECURITY_SERVICE_PORT`, public (no truststore) on `SECURITY_SERVICE_PUBLIC_PORT`. The public connector accepts anonymous handshakes; the allow-list enforces that only the two public routes are served there. | `adapters/inbound/http/auth/MtlsAuthPlugin.kt`, `infrastructure/Application.kt::buildServer` |
...
|---------|----------|-------|
| AC-3 (access enforcement) | ✅ | mTLS + admin allow-list. Two documented exceptions on the public allow-list: `GET /v1/jwks` (RFC 7517 public key material), `GET /v1/health` (k8s probe). JWKS is per-IP rate-limited. |
